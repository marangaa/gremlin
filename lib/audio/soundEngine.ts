...
export class SoundSynthesizer {
   ...
   * Generates a rapid sequence of frequency-modulated formant chirps
   * tailored to each character's pitch and timbre.
   */
  public playAnimalese(text: string, organismId: OrganismId = 'nexus') {
    ...
    switch (organismId) {
      case 'aero': // Momo (Sweet, high pitch)
        basePitch = 660;
        waveType = 'sine';
        break;
      case 'kuro': // Kuro (Low, raspy, mischievous)
        basePitch = 240;
        waveType = 'sawtooth';
        break;
      case 'cipher': // Bolt (Robotic, crisp)
        basePitch = 380;
        waveType = 'square';
        break;
      case 'atlas': // Glitch (8-bit retro pulse)
        basePitch = 520;
        waveType = 'square';
        break;
      case 'nexus': // Gorg (Alien trill)
      default:
        basePitch = 480;
        waveType = 'triangle';
        break;
    ...
    const syllableDuration = 0.05; // 50ms per chirp
    ...
    for (let i = 0; i < syllables; i++) {
      ...
      filter.type = 'lowpass';
      filter.frequency.setValueAtTime(organismId === 'cipher' || organismId === 'atlas' ? 2400 : 1600, noteStart);
