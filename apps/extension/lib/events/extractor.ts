...
export class PageContentExtractor {
  ...
  public extract(): PageSignalSnapshot | null {
    ...
    if (mainContainer) {
      ...
      articleSnippet = sanitized.slice(0, 500);
    ...
    // 5. HTML5 Audio/Video Activity
    const mediaElements = Array.from(document.querySelectorAll('video, audio')) as HTMLMediaElement[];
    const isMediaPlaying = mediaElements.some((media) => !media.paused && !media.ended && media.currentTime > 0);
